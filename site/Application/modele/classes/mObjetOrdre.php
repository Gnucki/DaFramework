...
class MObjetOrdre extends MObjetMetier
{
	protected $colOrdre;

	// Colonnes dont dépend l'ordre.
	public function __construct($id = NULL, $ordre = NULL, $colOrdre = array())
	{
		$this->SetObjet($id, $ordre, $colOrdre);
	}
	...
	public function Ajouter()
	{
		...
		if (is_string($this->colOrdre))
			$this->AjouterColCondition($this->colOrdre);
		else if (is_array($this->colOrdre))
		{
			foreach ($this->colOrdre as $col)
			{
				$this->AjouterColCondition($col);
			}
		}
