...
if (GDroit::EstConnecte(true) === true)
{
	...
	if ($dejaCharge === false)
	{
		...
		$form->FusionnerCelluleCadre(1, 1, 0, 5);
		$form->FusionnerCelluleCadre(2, 1, 0, 5);
		...
		$form->AjouterInputText(1, 1, GSession::Libelle(LIB_GPE_NOM), '', true, GContexte::FormaterVariable($nomContexte, COL_NOM), '', 1, 100, 50, false, '', GSession::Libelle(LIB_GPE_NOMINFO), GSession::Libelle(LIB_GPE_NOMERREUR), INPUTTEXT_REGEXP_TOUT_FV);
		$select = $form->AjouterInputSelect(2, 1, GSession::Libelle(LIB_GPE_JEU), INPUTSELECT_TYPE_FIND, true, GContexte::FormaterVariable($nomContexte, COL_JEU), GSession::Libelle(LIB_GPE_JEUINFO), GSession::Libelle(LIB_GPE_JEUERREUR), $nomContexte.COL_JEU, $nomContexte.COL_SERVEUR.';'.$nomContexte.COL_TYPEGROUPE, '', AJAXFONC_CHARGERREFERENTIELCONTEXTE, 'contexte='.$nomContexte);
		$select->AjouterElementsFromListe(COL_JEU, COL_ID, array(COL_LIBELLE, COL_LIBELLE), '', $jeuId);
		$select = $form->AjouterInputSelect(3, 5, GSession::Libelle(LIB_GPE_SERVEUR), '', false, GContexte::FormaterVariable($nomContexte, COL_SERVEUR), '', '', $nomContexte.COL_SERVEUR, '', $nomContexte.COL_JEU, AJAXFONC_CHARGERREFERENTIELCONTEXTE, 'contexte='.$nomContexte);
		...
		$select = $form->AjouterInputSelect(3, 1, GSession::Libelle(LIB_GPE_TYPEGROUPE), '', true, GContexte::FormaterVariable($nomContexte, COL_TYPEGROUPE), GSession::Libelle(LIB_GPE_TYPEGPEINFO), GSession::Libelle(LIB_GPE_TYPEGPEERREUR), $nomContexte.COL_TYPEGROUPE, '', $nomContexte.COL_JEU, AJAXFONC_CHARGERREFERENTIELCONTEXTE, 'contexte='.$nomContexte);
